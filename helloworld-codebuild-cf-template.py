...
environment = Environment(
	...
	EnvironmentVariables=[
		...
		{'Name': 'REPOSITORY_URI', 'Value': Join("", [
...
phases:
	...
	post_build:
		...
		- aws ecr batch-get-image --repository-name $REPOSITORY_NAME --imageids	imageTag="$(cat /tmp/tag.txt)" --query 'images[].imageManifest' --output text | tee /tmp/latest_manifest.json
